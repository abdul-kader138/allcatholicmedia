...
class DonationController extends Controller
{
    ...
    public function guestForm(): Response
    {
        SeoHelper::setTitle('Donate');

        return Theme::scope('donations.guest-form', [
            'presetAmounts' => self::PRESET_AMOUNTS,
        ])->render();
    }

    public function guestInitiate(Request $request): RedirectResponse
    {
        $request->validate([
        ...
        try {
            ...
            $order = $provider->createOrder([
                ...
                'application_context' => [
                    ...
                    'landing_page' => 'BILLING',
                    'user_action' => 'PAY_NOW',
                    'return_url'  => route('donation.guest.return', ['donation' => $donation->getKey(), 'token' => $token]),
                    'cancel_url'  => route('donation.guest.cancel', ['donation' => $donation->getKey(), 'token' => $token]),
                ],
                'purchase_units' => [[
                    'reference_id' => (string) $donation->getKey(),
                    'description'  => 'Donation to ' . config('app.name'),
            ...
            return back()->withInput()->withErrors(['paypal' => 'Unable to connect to PayPal. Please try again.']);
        } catch (Throwable $e) {
            $donation->update(['status' => 'failed']);
            report($e);

            return back()->withInput()->withErrors(['paypal' => 'PayPal is currently unavailable. Please try again later.']);
        }
    }

    public function guestReturn(Request $request, Donation $donation, string $token): View|Response|RedirectResponse
    {
        if ($donation->getAttribute('guest_token') !== $token || $donation->getAttribute('member_id') !== null) {
            abort(403);
        }

        if ($donation->getAttribute('status') === 'completed') {
            return Theme::scope('donations.guest-success', compact('donation'))->render();
        }

        if ($donation->getAttribute('status') !== 'pending') {
            return redirect()->route('donation.guest.form')->withErrors(['paypal' => 'This donation is no longer valid.']);
        }
        ...
        if (! $paypalToken) {
            ...
            return redirect()->route('donation.guest.form')->withErrors(['paypal' => 'Invalid PayPal response.']);
        }

        try {
            $provider      = $this->makePayPalClient();
            $capturedOrder = $provider->capturePaymentOrder($paypalToken);

            if (isset($capturedOrder['status']) && $capturedOrder['status'] === 'COMPLETED') {
                $capture = $capturedOrder['purchase_units'][0]['payments']['captures'][0] ?? [];
            ...
            return redirect()->route('donation.guest.form')->withErrors(['paypal' => 'Payment could not be completed. Please try again.']);
        } catch (Throwable $e) {
            $donation->update(['status' => 'failed']);
            report($e);

            return redirect()->route('donation.guest.form')->withErrors(['paypal' => 'A payment error occurred. Contact support if your account was charged.']);
        }
    }

    public function guestCancel(Donation $donation, string $token): RedirectResponse
    {
        if ($donation->getAttribute('guest_token') !== $token || $donation->getAttribute('member_id') !== null) {
            abort(403);
        }
        ...
        return redirect()->route('donation.guest.form')->with('info', 'Your donation was cancelled. You were not charged.');
    }
