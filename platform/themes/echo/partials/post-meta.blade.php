@php
    ...
    $isShowAuthorName = ($isSingle || echo_is_show_post_meta('author_name'));
    ...
    <div @class(['echo-3-align-icons post-meta', $wrapperClass])>
        @if ($isShowAuthorName)
            <div class="echo-hero-area-like-read-comment-share mb-2">
                @if ($author && $author->name && is_plugin_active('member') && $author instanceof \Botble\Member\Models\Member)
                    <a href="{{ $author->url }}" title="{{ $author->name }}">
                        <x-core::icon name="ti ti-user" class="me-1" />
                        <span>{{ $author->name }}</span>
                    </a>
                @elseif ($author && $author->name)
                    <a href="{{ url('/about-us') }}" title="Morson Livingston">
                        <x-core::icon name="ti ti-user" class="me-1" />
                        <span>{{ $author->name }}</span>
                    </a>
                @else
