...
<style>
.wlr-grid{position:relative;z-index:2;max-width:1560px;margin:0 auto;padding:0 40px;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:28px}
.wlr-card{border-radius:18px;padding:36px 40px 44px;display:flex;flex-direction:column;gap:0;min-width:0}
.wlr-card__head{display:flex;align-items:center;gap:18px;margin-bottom:22px}
.wlr-card__head h3{word-break:normal;overflow-wrap:anywhere}
.wlr-card__body{word-break:normal;overflow-wrap:anywhere}
@media (max-width: 991px){
    .wlr-grid{grid-template-columns:1fr;gap:18px;padding:0 20px}
    .wlr-card{padding:24px 22px 26px}
    .wlr-card__head{gap:14px;margin-bottom:16px}
}
@media (max-width: 575px){
    .watch-listen-read-section{padding:44px 0!important}
    .wlr-grid{padding:0 14px}
    .wlr-card{border-radius:16px;padding:20px 16px 22px}
    .wlr-card__head{align-items:center;gap:12px;margin-bottom:14px}
    .wlr-card__head svg{height:30px;width:30px}
    .wlr-card__head h3{font-size:1.45rem!important;line-height:1.1!important}
    .wlr-card__body{font-size:.96rem!important;line-height:1.6!important}
}
</style>
...
<section class="watch-listen-read-section" style="position: relative; width: 100%; margin: 0 !important; overflow: hidden; {{ $bgImage ? 'background-image: url(\'' . $bgImage . '\');' : 'background-color: #111827;' }} background-size: cover; background-position: center; background-repeat: no-repeat; padding: 90px 0;">
    ...
    <div class="wlr-grid">
        ...
        <div class="wlr-card" style="background: {{ $card['color'] }};">

            {{-- Icon + Title row --}}
            <div class="wlr-card__head">
            ...
            {{-- Description --}}
            <p class="wlr-card__body" style="font-family:'Inter',sans-serif!important;font-size:clamp(1.1rem,1.4vw,1.35rem)!important;font-weight:400!important;color:rgba(255,255,255,0.92)!important;line-height:1.6!important;margin:0!important;padding:0!important;background:transparent!important;">{{ $card['desc'] }}</p>
