...
if (! function_exists('echo_politics_default_saint_image_url')) {
    function echo_politics_default_saint_image_url(): string
    {
        return asset('themes/echo-politics/images/default-saint-card.svg');
    }
}

if (! function_exists('echo_politics_extract_first_image_from_html')) {
    function echo_politics_extract_first_image_from_html(?string $html): ?string
    {
        if (! $html) {
            return null;
        }
...
if (! function_exists('echo_politics_extract_video_url_from_html')) {
    function echo_politics_extract_video_url_from_html(?string $html): ?string
    {
        if (! $html) {
            return null;
        }
...
if (! function_exists('echo_politics_extract_audio_url_from_html')) {
    function echo_politics_extract_audio_url_from_html(?string $html): ?string
    {
        if (! $html) {
            return null;
        }
...
if (! function_exists('echo_politics_youtube_thumbnail_url')) {
    function echo_politics_youtube_thumbnail_url(?string $videoUrl): ?string
    {
        if (! $videoUrl || ! Youtube::isYoutubeURL($videoUrl)) {
            return null;
        }
...
if (! function_exists('echo_politics_resolve_post_media')) {
    function echo_politics_resolve_post_media(Post $post, string $imageSize = 'medium'): array
    {
        $content = (string) ($post->content ?? '');
        $videoUrl = function_exists('echo_get_post_video_url') ? echo_get_post_video_url($post) : null;
        $audioUrl = function_exists('echo_get_post_audio_url') ? echo_get_post_audio_url($post) : null;

        $videoUrl = $videoUrl ?: echo_politics_extract_video_url_from_html($content);
        $audioUrl = $audioUrl ?: echo_politics_extract_audio_url_from_html($content);

        if (! empty($post->image)) {
            return [
                'image_url' => RvMedia::getImageUrl($post->image, $imageSize),
        ...
        $contentImage = echo_politics_extract_first_image_from_html($content);
        ...
        $videoThumb = echo_politics_youtube_thumbnail_url($videoUrl);

        if ($videoThumb) {
            return [
                'image_url' => $videoThumb,
                'media_type' => 'video',
                'video_url' => $videoUrl,
                'audio_url' => $audioUrl,
                'uses_default' => false,
        ...
        return [
            'image_url' => echo_politics_default_saint_image_url(),
            'media_type' => $audioUrl ? 'audio' : ($videoUrl ? 'video' : 'default'),
            'video_url' => $videoUrl,
            'audio_url' => $audioUrl,
            'uses_default' => true,
        ];
...
app()->booted(function (): void {
    ...
    add_filter(THEME_FRONT_HEADER, function (?string $html): string {
        ...
        $html .= '<link rel="stylesheet" href="' . asset('themes/echo-politics/css/footer-newsletter.css') . '?ver=1">' . PHP_EOL;
...
@media(max-width:768px){
    ...
    Shortcode::setAdminConfig('hero-intro', function (array $attributes) {
        $form = ShortcodeForm::createFromArray($attributes)
            ...
            ->add('show_cards', SelectField::class,
                SelectFieldOption::make()
                    ->label(__('Show Bottom Cards'))
                    ->choices(['yes' => 'Yes — show cards', 'no' => 'No — hide cards'])
                    ->defaultValue('yes')
                    ->toArray()
            );
    ...
    Shortcode::register('watch-learn-pray', __('Watch Learn Pray'), __('Display building faith section'), function (ShortcodeCompiler $shortcode): ?string {
        add_filter(THEME_FRONT_HEADER, function (?string $html): string {
            ...
            if (strpos($html, 'Great+Vibes') === false) {
                $html .= '<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Great+Vibes&family=Inter:wght@400;500;600;700&family=Playfair+Display:wght@700&display=swap">' . PHP_EOL;
            }
            return $html;
        }, 25, 1);
