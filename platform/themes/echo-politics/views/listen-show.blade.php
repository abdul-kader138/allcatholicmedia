...
html[data-theme='dark'] {
    --lv-page: #0d1117;
    --lv-card: #141a27;
    --lv-border: rgba(255, 255, 255, .08);
    --lv-heading: #e8f0fe;
    --lv-body: rgba(226, 232, 240, .78);
    --lv-muted: #64748b
}
...
.acm-lshow-back:hover {
    color: var(--lv-gold);
    text-decoration: none
}

.acm-lshow-header {
    background: linear-gradient(135deg, #0d1b0e 0%, #1a3320 52%, #0d1b0e 100%);
    border-radius: 22px;
    overflow: hidden;
    margin-bottom: 28px;
    color: #f8fbff
...
.acm-lshow-embed iframe {
    width: 100%;
    border: 0;
    display: block
}

.acm-lshow-embed--soundcloud iframe {
    height: 166px
}

.acm-lshow-embed--youtube {
    aspect-ratio: 16/9
}

.acm-lshow-embed--youtube iframe {
    width: 100%;
    height: 100%
}
...
.acm-ep-card:hover {
    ...
    border-color: rgba(40, 140, 60, .22);
    text-decoration: none
}

.acm-ep-card--active {
    border-color: rgba(201, 162, 39, .55);
    box-shadow: 0 0 0 2px rgba(201, 162, 39, .18), 0 16px 34px rgba(15, 23, 42, .10)
}
...
.acm-ep-pill--play {
    background: rgba(45, 138, 62, .9);
    color: #fff
}

.acm-ep-pill--featured {
...
document.addEventListener('DOMContentLoaded', function() {
    function injectNativeListenAudio() {
        document.querySelectorAll('.acm-lshow-audio-wrap[data-audio-url]').forEach(function(wrap) {
            if (wrap.querySelector('audio.acm-lshow-audio')) {
                return;
            }

            const audioUrl = wrap.getAttribute('data-audio-url');
            const audioType = wrap.getAttribute('data-audio-type') || 'audio/mpeg';
            ...
            audio.className = 'acm-lshow-audio';
    ...
    window.addEventListener('load', injectNativeListenAudio);
    setTimeout(injectNativeListenAudio, 300);
    setTimeout(injectNativeListenAudio, 900);
});
</script>
...
<div class="acm-lshow">
    <div class="container acm-lshow-shell">
        ...
        <section class="acm-lshow-player">
            ...
            <p class="acm-lshow-player-desc">{{ \Illuminate\Support\Str::limit($selectedEpisode->description, 300) }}
            </p>
            @endif

            @if ($selectedEpisode->embed_type === 'html5' && $selectedEpisode->audio_url)
            <div class="acm-lshow-audio-wrap" data-audio-url="{{ $selectedEpisode->audio_url }}"
                data-audio-type="audio/mpeg">
                <div class="acm-lshow-audio-placeholder">{{ __('Loading audio player...') }}</div>
            </div>
            ...
            <div class="acm-lshow-embed acm-lshow-embed--youtube">
                <iframe src="{{ $selectedEpisode->embed_url }}?autoplay=0&rel=0&modestbranding=1" allowfullscreen
                    title="{{ $selectedEpisode->title }}"></iframe>
            </div>
            @elseif ($selectedEpisode->audio_url)
            <a href="{{ $selectedEpisode->audio_url }}" target="_blank" rel="noopener noreferrer"
