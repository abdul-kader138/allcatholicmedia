...
.acm-listen-hero.has-bg-img::before{
    content:'';position:absolute;inset:0;
    background:linear-gradient(158deg,rgba(6,14,29,.82) 0%,rgba(13,31,60,.78) 42%,rgba(11,26,53,.80) 72%,rgba(5,12,24,.88) 100%);
    pointer-events:none;z-index:0
}
/* Radial gold glow — same as Saints / Live (only when no bg image) */
.acm-listen-hero:not(.has-bg-img)::before{
...
.acm-listen-sub{
    ...
    font-size:clamp(1.12rem,2vw,1.35rem);font-weight:300;
...
.acm-listen-stat{
    padding:12px 28px;
    font-family:'Inter',system-ui,sans-serif;font-size:.90rem;
...
<div class="acm-listen-page">
    <section class="acm-listen-hero {{ $heroImage ? 'has-bg-img' : '' }}"
             @if ($heroImage) style="background-image:url('{{ $heroImage }}');background-size:cover;background-position:center;" @endif>
